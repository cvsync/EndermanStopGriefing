# Enderman Stop-Griefing

The Minecraft data pack, ``Enderman Stop-Griefing,'' prevents Enderman from griefing items.

## Latest Version

1.11.2

## For Java Edition (1.21.9/1.21.10/1.21.11 or later) users

1.) Download `EndermanStopGriefing.zip`

2.) Put it into `C:\Users\<Username>\AppData\Roaming\.minecraft\saves\<World>\datapacks`

## For Java Edition (1.21.7/1.21.8) users

Try version 1.9.0, as these files are stored in the `1.9.0` directory.

## For Java Edition (1.21.6) users

Try version 1.8.0, as these files are stored in the `1.8.0` directory.

## For Java Edition (1.21.5) users

Try version 1.7.0, as these files are stored in the `1.7.0` directory.

## For Java Edition (1.21.4) users

Try version 1.6.0, as these files are stored in the `1.6.0` directory.

## For Java Edition (1.21.2/1.21.3) users

Try version 1.5.0, as these files are stored in the `1.5.0` directory.

## For Java Edition (1.21/1.21.1) users

Try version 1.4.3, as these files are stored in the `1.4.3` directory.

## For Java Edition (1.20.5/1.20.6) users

Try version 1.3.0, as these files are stored in the `1.3.0` directory.

## For Java Edition (1.20.3/1.20.4) users

Try version 1.2.0, as these files are stored in the `1.2.0` directory.

## For Java Edition (1.20.2) users

Try version 1.1.0, as these files are stored in the `1.1.0` directory.

## For Java Edition (1.20/1.20.1) users

Try version 1.0.0, as these files are stored in the `1.0.0` directory.
